...
To say InfluenceDesc of (C - a wearthing):
	...
	if the bbc-sex-addiction-influence of C < 0 and interracial fetish is 1, say "You can sense that this [item of C] is [if the bbc-sex-addiction-influence of C < -1]significantly [end if][if the BBC addiction of the player > 3]reducing your preference for [BlackCock][otherwise]helping you maintain your sexual preferences when it comes to race[end if].";
	if the bbc-sex-addiction-influence of C > 0 and interracial fetish is 1, say "You can sense that this [item of C] is making you feel [if the bbc-sex-addiction-influence of C > 1]a lot [end if]more [if the BBC addiction of the player < 6]interested in black [men of shopkeeper][otherwise]addicted to [BlackCock][end if].";
...
Report examining breast covering clothing:
	...
	if the noun is erect-nipple-exposing actually dense actually nipple covering clothing:
		...
		say "The thin nature of the fabric means that your [if the player is horny]erect [end if]nipples [if C is clothing]could[otherwise]can[end if] be seen poking through it[if the player is not horny] when erect[end if][if C is clothing] if it wasn't for [NameDesc of C][end if].";

To say selfexamineuniquetitle of (C - a clothing):
	say "[ShortDesc of C]".

Report examing worn audibly jiggling clothing:
